...
export function registerMcpCapabilities(
        ...
        async ({ query, count, language }) => {
            try {
                ...
                if (locations.length === 0) {
                    return textResult(`"${query}"에 해당하는 위치를 찾지 못했습니다.`, {
                        locations: []
                    })
                }
                ...
                return textResult(`"${query}" 검색 결과 ${locations.length}건:\n${summary}`, {
                    locations
                })
            } catch (error) {
                const message =
    ...
    server.registerTool(
        'calculator',
        {
            description: '두 숫자와 연산자(+, -, *, /)를 입력받아 계산 결과를 반환합니다.',
            inputSchema: z.object({
    ...
    server.registerTool(
        ...
        async ({ latitude, longitude, forecastDays }) => {
            try {
                ...
                const message =
                    error instanceof Error ? error.message : '알 수 없는 오류가 발생했습니다.'
                return textResult(`날씨 조회 실패: ${message}`, { latitude, longitude }, true)
            }
        }
    )
    ...
    server.registerTool(
        ...
        async ({ prompt, num_inference_steps }) => {
            if (!hfToken) {
                return textResult(
                    'x-hf-token 헤더 또는 HF_TOKEN 환경변수가 설정되어 있지 않습니다. HuggingFace API 토큰을 설정한 뒤 다시 시도하세요.',
